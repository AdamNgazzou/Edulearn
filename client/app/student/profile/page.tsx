...
export default async function ProfilePage() {
  ...
  const response = await fetch(`${process.env.BACKEND_PUBLIC_API_URL}/student/profile/${id}`)
  const profileData = await response.json()

  const response2 = await fetch(`${process.env.BACKEND_PUBLIC_API_URL}/student/profile/courses/${id}`)
